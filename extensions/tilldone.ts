...
/* 
我的笔记
为什么需要用 TypeBox: 因为 TypeScript 是静态类型检查，编译后会被擦除，无法在运行时进行验证。而 AI Agent 工具，接收用户输入的参数时，需要运行时保证参数符合预期结构。所以需要使用 TypeBox。
*/
const TillDoneParams = Type.Object({ // Type.Object({}) 定义一个对象类型的 schema，每个字段对应一个属性
    action: StringEnum(["new-list", "add", "toggle", "remove", "update", "list", "clear"] as const), // StringEnum() 定义一个枚举类型的 schema，表示 action 只能是这些值之一
...
// 我的笔记 - 这段代码 Java 味好浓（Java 必须事先声明所有字段和类型，现代 TypeScript 可以省略，但是这里为了严谨性和可读性还是声明了）
class TillDoneListComponent {
    ...
    // 我的笔记 - 2. 构造函数（相当于 Python 中的 def __init__(self)）；也可以直接将类型声明放到构建函数里，如 constructor(private tasks: Task[], ...) 这样就不用再写 this.tasks = tasks 了
    ...
    // 我的笔记 - 在类里面，成员函数采用简写，不需要写 function handleInput
    // 我的笔记 - 我注意到这个方法只是声明了一下。猜测 Mario 在 Pi 的底层中定义了一套契约，规定了如果通过 ctx.ui.custom 传给 pi 一个对象，只要这个对象上面有 handleInput 这个方法，那么会负责侦听系统终端的原始按键，然后将按键数据传给这个方法
    ...
    // 我的笔记 - `invalidate` 真正的使命是“数据同步”。在两种情况下会被调用：1. 核心应用场景 - 数据变了，但是显示没关（比如打开着 tilldone 任务列表，但是后台，比如LLM更新了任务进程）；2. 在本文件中的实际调用，如后面代码中的 container.invalidate() 
    // 我的笔记 - 启发：实时更新的研读进度条
    invalidate(): void {
...
export default function (pi: ExtensionAPI) {
    ...
    // 我的笔记 - 标记在本轮会话（agent_end）是否已经催促过用户
    ...
    const reconstructState = (ctx: ExtensionContext) => {
        ...
        for (const entry of ctx.sessionManager.getBranch()) {
            ...
            // 🚨 核心修复：如果由于中断（如 Ctrl+C）或框架异常，Pi 写了一个空的 details: {}
            // 这里仅仅判断 if (details) 是不够的，会导致 tasks = undefined！
            // 必须强制验证 details.tasks 是否为合法数组
    ...
    pi.on("tool_call", async (event, _ctx) => {
        // 我的笔记：调用 tilldone 工具，一律放行 
        if (event.toolName === "tilldone") return { block: false };

        // 我的笔记：调用其他工具，调查未完成的任务和进行中的任务
        ...
        // 我的笔记：所有任务都已完成，调用其他工具，阻塞！理由是必须添加新任务，或者新的任务清单。
        if (pending.length === 0) {
            return {
                block: true,
        ...
        // 我的笔记：没有任务正在进行中，调用其他工具，阻塞！理由是必须添加新任务，或者新的任务清单。
        if (active.length === 0) {
            return {
                block: true,
    ...
    // 我的笔记 - agent_end 在每个用户 prompt 处理完成后触发。
    pi.on("agent_end", async (_event, _ctx) => {
        ...
        pi.sendMessage(
            ...
            // triggerTurn: true 表示在发送这个消息后，立即进入下一个用户 turn，从而产生 turn_start、turn_end、agent_end 等事件，而不是等待用户回复
            /*
             - triggerTurn: true = 把这条消息交给 LLM 处理 → 会触发 turn_start / turn_end / agent_end
             - triggerTurn: false = 只是把消息加入对话历史，LLM 不会立即响应
            官方文档：If agent is idle, trigger an LLM response immediately.
            */
            { triggerTurn: true },
        );
    });
    // 我的笔记 - 用户输入被接收时触发，表示我开始新一轮对话了，又可以催促我了
    pi.on("input", async () => {
    ...
    pi.registerTool({
        name: "tilldone",
        label: "TillDone",
        description: // 用于 JSON Schema，给 LLM 看的“兵器知识概述”，另外还有 promptSnippet - 用于在 system prompt 的 "Available tools" 部分显示
            ...
            "Actions: new-list (text=title, description), add (text or texts[] for batch), toggle (id) — cycles idle→inprogress→done (CRITICAL: You MUST ONLY toggle a task to 'done' AFTER the user has explicitly confirmed the work is complete. NEVER finish tasks on your own initiative without human verification. If the user doesn't specify an ID, ask for clarification instead of guessing.), remove (id), update (id + text), list (CRITICAL: Whenever you need to show the tasks to the user, you MUST call this 'list' action to get a fresh snapshot from the session details), clear. " +
        ...
        // execute 函数是工具的实际执行逻辑，当 LLM 调用这个工具时，execute 就会被触发并执行具体操作
        async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
            // params - LLM 传入的参数
            // toolCallId - 调用 ID，用于追踪
            // signal - abort 信号（用户可以取消）
            // onUpdate - 进度更新（可以流式返回结果）
            // ctx - 扩展上下文
            switch (params.action) {
                case "new-list": {
                    ...
                    if (tasks.length > 0 || listTitle) {
                        ...
                        if (!confirmed) {
                            return {
                                // 我的笔记 - const 断言。如果不加上 as const，TypeScript 会将它推断为普通的 string 类型，而 string 类型太宽了，无法匹配 pi 要求的精确的 "text" 的字面量类型
        ...
        /*
        我的笔记
        renderCall：调用工具时，将一串 JSON 转换为人类可以直接阅读、带颜色的精美标题行
        根据 extension.md，这可选重写函数，用于自定义渲染
        根据 tui.md，在 renderCall / renderResult 中，使用 theme 参数
        Use theme.fg() for foreground colors
        Use theme.bg() for background colors
        return new Text(theme.fg("success", "Done!"), 0, 0);
        */
        renderCall(args, theme) {
            // 我的笔记 - toolTitle、muted、dim、accent 都是颜色标记
            // 绘制前置        用toolTitle颜色  加粗的 "tilldone"文字 +  muted 颜色的 action 文字
            let text = theme.fg("toolTitle", theme.bold("tilldone ")) + theme.fg("muted", args.action);
            // 参数中的 texts ≥ 1，在 text 后缀 dim 颜色的 {n} tasks 文字，比如 tilldone add 2 tasks
            if (args.texts?.length) text += ` ${theme.fg("dim", `${args.texts.length} tasks`)}`;
            // 参数中 text 不为空，在 text 后缀 dim 颜色的 "{text}" 文字，比如 tilldone add "梳理与 TUI 交叉的知识点"
            else if (args.text) text += ` ${theme.fg("dim", `"${args.text}"`)}`;
            // 参数中 description 不为空，在 text 后缀 dim 颜色的 "— {description}" 文字，比如 tilldone new-list "和我闲聊" — 和我闲聊的四大重点八大要义
            if (args.description) text += ` ${theme.fg("dim", `— ${args.description}`)}`;
            // 参数中 id 不为空，在 text 后缀 accent 颜色的 "#{id}" 文字，比如 tilldone update 1
            if (args.id !== undefined) text += ` ${theme.fg("accent", `#${args.id}`)}`;
            return new Text(text, 0, 0);
        },

        // 我的笔记，根据 extension.md，这可选重写函数，用于自定义渲染调用工具工具产生的结果，也就是 toolResult
        renderResult(result, { expanded }, theme) {
            const details = result.details as TillDoneDetails | undefined;
            // 同理，渲染层也要防御损坏的空 details 载荷
            if (!details || !Array.isArray(details.tasks)) {
    ...
    pi.registerCommand("tilldone", {
        ...
        handler: async (_args, ctx) => {
            ...
            await ctx.ui.custom<void>((_tui, theme, _kb, done) => {
                return new TillDoneListComponent(tasks, listTitle, listDescription, theme, () => done()); // () => done()，调用框架提供的 done()，关闭这个组件
            });
        },
    });
}
